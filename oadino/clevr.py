...
def load_clevr_dataset(images_path: Path, maxsize, transform):
    ...
    # Load and transform
    train_dataset = create_clevr_hf_dataset(images_path / "train", maxsize=4096)
    test_dataset = create_clevr_hf_dataset(images_path / "test", maxsize=4096)
    val_dataset = create_clevr_hf_dataset(images_path / "val", maxsize=4096)
...
def load_clevertex_dataset(base_dir, max_size, transform, seed=42):
    ...
    def create_split_dataset(paths):
        data = {
            'image': [path for path, _ in paths],
